...
export async function getAdminStats(): Promise<AdminStats> {
  ...
  return {
    companiesTotal: Number(
      data.companies_total ?? 0,
    ),
    companiesCreatedThisMonth: Number(
      data.companies_created_this_month ?? 0,
    ),
    subscriptionsActive: Number(
      data.subscriptions_active ?? 0,
    ),
    subscriptionsTrialing: Number(
      data.subscriptions_trialing ?? 0,
    ),
    subscriptionsPastDue: Number(
      data.subscriptions_past_due ?? 0,
    ),
    starterCount: Number(
    ...
    estimatedMrrEur: Number(
      data.estimated_mrr_eur ?? 0,
    ),
    openTickets: Number(
      data.open_tickets ?? 0,
    ),
    urgentTickets: Number(
      data.urgent_tickets ?? 0,
    ),
  }
}
...
Promise<AdminCompany[]> {
  ...
  return (data ?? []).map(
    (row: Record<string, unknown>) => ({
      ...
      planId: String(
        row.plan_id ?? 'business',
      ) as AdminCompany['planId'],
      planName: String(
        row.plan_name ?? 'Business',
      ),
      subscriptionStatus: String(
