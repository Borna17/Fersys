    ...
    <section className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">Pregled server-side verzija nespremljenog rada. Snapshoti su samo za oporavak; ova stranica ne prepisuje aktivne podatke automatski.</p>
        </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[900px] text-left text-sm">
              ...
              <tbody>{filtered.map((row) => <tr key={row.id} className="border-t border-slate-800"><td className="p-4 whitespace-nowrap">{new Date(row.created_at).toLocaleString('hr-HR')}</td><td className="p-4"><div className="font-bold">{row.user_email || row.user_id}</div><div className="mt-1 text-xs text-slate-500">{row.company_id}</div></td><td className="p-4">{row.draft_type}</td><td className="p-4 font-mono text-xs">{row.draft_key}</td><td className="p-4">{row.reason}</td><td className="p-4"><div className="flex gap-2"><button type="button" onClick={() => setSelected(row)} className="rounded-lg bg-slate-800 px-3 py-2 font-bold">Pregled</button><button type="button" onClick={() => void copyPayload(row)} className="rounded-lg bg-violet-600/20 px-3 py-2 font-bold text-violet-300"><Copy size={15} /></button></div></td></tr>)}</tbody>
            </table>
          </div>
