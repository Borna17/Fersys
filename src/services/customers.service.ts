...
function mapCustomer(row: CustomerRow): Customer {
  return {
    ...
    logo: row.logo_data_url ?? undefined,
    oib: row.oib,
    phone: row.phone ?? '',
    email: row.email ?? '',
    street: row.street ?? '',
    ...
    iban: row.iban ?? '',
    notes: row.notes ?? '',
    workOrders: Number(row.work_orders_count ?? 0),
...
export async function getCustomerById(
  ...
  return data
    ? mapCustomer(data as CustomerRow)
    : null
}
