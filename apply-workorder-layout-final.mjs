...
try {
  // ------------------------------------------------------------
  // 1. PAGINACIJA
  // - prva stranica je glavni radni nalog
  // - do 3 fotografije idu na prvu stranicu samo kad stvarno ima mjesta
  // - ako ima više sadržaja, fotografije automatski idu na sljedeću stranicu
  // - preostale fotografije idu u grupe po 4
  // ------------------------------------------------------------
  ...
  /*
   * Prva stranica mora ostati čitljiva.
   *
   * 0–5 stavki materijala + kraći opis:
   *   -> do 3 fotografije mogu ostati na prvoj stranici.
   *
   * Više materijala ili duži opis:
   *   -> prva stranica koristi prostor za tekst i materijal,
   *      a fotografije prelaze na sljedeću stranicu.
   */
  ...
  /*
   * Ako na prvoj stranici imamo fotografije,
   * ostavljamo manje redaka materijala da ništa ne bude nagurano.
   */
...
`
  ...
  // ------------------------------------------------------------
  // 2. MINIMAL više nije CUSTOM.
  // Modern, Classic i Minimal imaju svaki svoj izgled.
  // ------------------------------------------------------------
  const oldLayoutClass = `function layoutClass(
  ...
  replaceExact(
    oldLayoutClass,
    newLayoutClass,
    'Modern / Classic / Minimal preset',
  )

  // ------------------------------------------------------------
  // 3. Završni CSS override.
  // Dodajemo ga pred @media print da sigurno pregazi stare PRO V2 vrijednosti.
  // ------------------------------------------------------------
  ...
  const finalCss = `
    ...
    /* ==========================================================
       ...
       Ovaj blok vrijedi samo za workOrderPdf.ts.
       Ponude i računi nisu povezani s ovim CSS-om.
       ========================================================== */
    ...
    /*
     * Gornji meta blok ostaje u jednom redu.
     * Font je veći, ali stupci su bolje raspoređeni.
     */
    ...
    /*
     * OPIS je namjerno kompaktniji.
     * Ne dobiva veliku praznu visinu.
     * Ako je tekst dug, raste samo koliko stvarno treba.
     */
    .section {
      margin-top: 8px;
    }
    ...
    /*
     * Materijal je važniji od praznog prostora.
     */
    table {
      font-size: 9px;
    }
     ...
     * - 1 slika: velika
     * - 2 slike: 2 stupca
     * - 3 slike: 3 stupca, taman za prvu stranicu
     * - 4 slike: 2 x 2 na nastavku
     */
    .photo-grid {
      gap: 8px;
    }
    ...
    /*
     * Potpis i pečat.
     * mix-blend-mode:multiply na bijelom PDF-u vizualno uklanja
     * bijelu pozadinu skeniranog pečata.
     */
    .signature-section {
      margin-top: auto;
...
`
  ...
  if (
    ...
    if (!text.includes(printMarker)) {
      throw new Error('Nije pronađen @media print završetak CSS-a.')
    }
    ...
    console.log('✓ Finalni Work Order CSS')
  } else {
    console.log('• Finalni Work Order CSS: već postoji')
  }

  // ------------------------------------------------------------
  // 4. photo-count klasa ako je još nema.
  // ------------------------------------------------------------
  replaceExact(
...
`      <div class="photo-grid photo-count-${photos.length}">
  ...
  console.error('Nijedna datoteka nije spremljena nakon ove greške.')
  process.exit(1)
}
